...
function buildWhatsAppShare(referralLink) {
  const msg = encodeURIComponent(
    `Hey! I use FameMap to get Google reviews on autopilot for my business. First 7 days are free — check it out: ${referralLink}`
  );
...
function buildSMSShare(referralLink) {
  const msg = encodeURIComponent(
    `Hey! Try FameMap — it automatically asks your customers for Google reviews. 7-day free trial: ${referralLink}`
  );
  return `sms:?body=${msg}`;
}
...
console.log('%cFameMap app.js loaded ✅', 'color:#4ade80;font-weight:bold;');
